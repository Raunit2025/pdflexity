...
// findPython locates a Python 3 interpreter, prioritizing the local virtual environment
func findPython() string {
	cwd, _ := os.Getwd()
	exePath, _ := os.Executable()
	exeDir := filepath.Dir(exePath)

	// 1. Look for the local virtual environment first
	venvCandidates := []string{
		// When running from root
		filepath.Join(cwd, "services", "ocr-engine", "venv", "Scripts", "python.exe"),
		filepath.Join(cwd, "services", "ocr-engine", "venv", "bin", "python"),
		// When running from apps/electron/bin/pdflexity-engine.exe
		filepath.Join(exeDir, "..", "..", "..", "services", "ocr-engine", "venv", "Scripts", "python.exe"),
		filepath.Join(exeDir, "..", "..", "..", "services", "ocr-engine", "venv", "bin", "python"),
	}

	for _, p := range venvCandidates {
		if _, err := os.Stat(p); err == nil {
			return p // Found the venv!
		}
	}

	// 2. Fallback to Global Python installations
